...
while IFS=, read category type key value
do
    ...
    sudo -n -u postgres -s -- psql -d ${POI_DB} -c "
        INSERT INTO poi
        SELECT osm_id, geom, name, '${category}', '${type}'
    ...
    sudo -n -u postgres -s -- psql -d ${POI_DB} -c "
        INSERT INTO poi
        SELECT osm_id, ST_Centroid(geom), name, '${category}', '${type}'
        FROM dblink('dbname=${OSM_DB}',
                    'SELECT osm_id, way, name FROM planet_osm_polygon WHERE ${key}=''${value}''')
              AS t1(osm_id bigint, geom geometry,name text);"
done < features.conf
